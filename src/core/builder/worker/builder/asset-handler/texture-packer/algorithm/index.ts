...
import MaxRectsBinPack, { IRect } from './maxrects';
...
function getRectsFromInputs(inputs: IInputRect[]): (IRect & { origin: IInputRect })[] {
    return inputs.map((r) => {
        return { width: r.width, height: r.height, origin: r } as IRect & { origin: IInputRect };
    });
}

function getInputsFromRects(rects: (IRect & { origin: IInputRect })[]): IPackedRect[] {
...
function scoreMaxRects(inputs: (IRect & { origin: IInputRect })[], binWidth: number, binHeight: number, heuristice: number, allowRotation: boolean, result: IScorePackResult): void {
...
function scoreMaxRectsForAllHeuristics(inputs: IInputRect[], binWidth: number, binHeight: number, allowRotation: boolean, result: IScorePackResult): void {
    for (let i = 0; i <= 5; i++) {
...
export const TexturePackerAlgorithm = {
